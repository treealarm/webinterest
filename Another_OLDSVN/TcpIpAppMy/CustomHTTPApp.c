...
/****************************************************************************
  ...
  ***************************************************************************/
		...
		static HTTP_IO_RESULT HTTPPostSettings(void);
...
BYTE cur_var_name[20]={0};
BYTE cur_var_val[20]={0};
/****************************************************************************
  Section:
...
BYTE HTTPCheckAuth(BYTE* cUser, BYTE* cPass)
{
	...
	if(
		strcmp((char*)cUser, (char*)AutonomSettings.data[E_NAME]) == 0 && 
		strcmp((char*)cPass, (char*)AutonomSettings.data[E_PASS]) == 0
	)
		return 0x80;		// We accept this combination
...
HTTP_IO_RESULT HTTPExecuteGet(void)
{
	...
	if(!memcmppgm2ram(filename, "protect/cards.htm", 17))
	{
		ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"card_position");
    	ptr1 = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"facility_code");
		ptr2 = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"card_code");
		if(ptr1 && ptr2)
		{
			SaveCard(atoi(ptr),Wiegand2TouchMemoryDword(ptr2,ptr1));
		}
	}

	else
	if(!memcmppgm2ram(filename, "vars.htm", 8))
	{
		ptr1 = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"var_name");
	   	ptr2 = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"var_val");
		if(ptr1 && ptr2)
		{
			for(i=0;i<8;i++)
			{
				sprintf(varname,"led%d",i);
				if(!strcmp((char*)ptr1, (char*)varname))
				{
					strcpy(cur_var_name,ptr1);
					strcpy(cur_var_val,ptr2);
					SetLed(i,(*ptr2 == '1'));
					ptr1 = 0;
					break;
				}
			...
			{	
				...
				sprintf(varname,"btn_change%d",i);
				if(!strcmp((char*)ptr1, (char*)varname))
				{
					if(i==0)
					{
						m_cur_button_states[i] = BUTTON0_IO;
						m_button_change[i] = 0;
					}
					if(i==1)
					{
						m_cur_button_states[i] = BUTTON1_IO;
						m_button_change[i] = 0;
					}
					if(i==2)
					{
						m_cur_button_states[i] = BUTTON2_IO;
						m_button_change[i] = 0;
					}
					if(i==3)
					{
						m_cur_button_states[i] = BUTTON3_IO;
						m_button_change[i] = 0;
					}
					strcpy(cur_var_name,ptr1);
					strcpy(cur_var_val,ptr2);
...
HTTP_IO_RESULT HTTPExecutePost(void)
{
	...
	if(!memcmppgm2ram(filename, "protect/setting.htm", 19))
...
static HTTP_IO_RESULT HTTPPostConfig(void)
{
	...
	SaveAppConfig((BYTE*)&newAppConfig);	
	//LoadAppConfig();
	#if defined(EEPROM_CS_TRIS)
...
{
	TCPPutString(sktHTTP,AutonomSettings.data[E_NAME]);
}

void HTTPPrint_pass(void)
{
	TCPPutString(sktHTTP,AutonomSettings.data[E_PASS]);
}

void HTTPPrint_port_cl(void)
{
	TCPPutString(sktHTTP,AutonomSettings.data[E_PORT_CL]);
}

void HTTPPrint_host_cl(void)
{
	TCPPutString(sktHTTP,AutonomSettings.data[E_HOST_CL]);
}

void HTTPPrint_var_name(void)
{
	TCPPutString(sktHTTP, (char*)cur_var_name);
}
void HTTPPrint_var_val(void)
{
	TCPPutString(sktHTTP, (char*)cur_var_val);
}
...
static HTTP_IO_RESULT HTTPPostSettings(void)
{
	...
	switch(curHTTP.smPost)
	{
		case SM_READ_NAME:
			...
			if(HTTPReadPostName(temp,SETTING_LEN2) == HTTP_READ_INCOMPLETE)
                return HTTP_IO_NEED_DATA;

			if(!strcmppgm2ram((char*)temp, (ROM char*)"name"))
			{
				AutonomSettings.dest = E_NAME;
			}
			else
			if(!strcmppgm2ram((char*)temp, (ROM char*)"pass"))
			{
				AutonomSettings.dest = E_PASS;
			}
			if(!strcmppgm2ram((char*)temp, (ROM char*)"host_cl"))
			{
				AutonomSettings.dest = E_HOST_CL;
			}
			else
			if(!strcmppgm2ram((char*)temp, (ROM char*)"port_cl"))
			{
				AutonomSettings.dest = E_PORT_CL;
			}
			else
			{
				AutonomSettings.dest = E_LAST;
			}	
		...
		case SM_READ_VALUE:
			// Read a value
			AutonomSettings.data[AutonomSettings.dest][0] = '\0';
			if(HTTPReadPostValue(&AutonomSettings.data[AutonomSettings.dest][0],SETTING_LEN2) == HTTP_READ_INCOMPLETE)
			{
                return HTTP_IO_NEED_DATA;
			}
...
void HTTPPrint_btn_change(WORD num)
{
// Print the output
	if(num < 4)
	{
		TCPPut(sktHTTP, (m_button_change[num]?'1':'0'));
	}
	else
	{
		TCPPut(sktHTTP, '?');
...
void HTTPPrint_pot(WORD port)
{
	if(port < ANALOG_INPUTS)
	{
		TCPPutString(sktHTTP, AN0String[port]);
	}
	else
	{
...
#define READER_TEMP 16
void HTTPPrint_reader(WORD num)
{
BYTE temp[READER_TEMP];
if(m_autonom)
{
	TCPPutROMString(sktHTTP, (ROM BYTE*)"autonom\n");
}
else
{
	TCPPutROMString(sktHTTP, (ROM BYTE*)"hosted\n");
}
if(m_skd_data.reader_state&READER_READY)
{
	TCPPutROMString(sktHTTP, (ROM BYTE*)"READER_READY|");
}
if(m_skd_data.reader_state&READER_KEY)
{
	TCPPutROMString(sktHTTP, (ROM BYTE*)"READER_KEY|");
}
if(m_skd_data.reader_state&READER_OPEN)
{
	TCPPutROMString(sktHTTP, (ROM BYTE*)"READER_OPEN|");
}
if(m_skd_data.reader_state&READER_CLOSE)
{
	TCPPutROMString(sktHTTP, (ROM BYTE*)"READER_CLOSE|");
}
if(m_skd_data.reader_state&READER_RTE)
{
	TCPPutROMString(sktHTTP, (ROM BYTE*)"READER_RTE|");
}
if(m_skd_data.reader_state&READER_RELAY)
{
	TCPPutROMString(sktHTTP, (ROM BYTE*)"READER_RELAY|");
}


TCPPutROMString(sktHTTP, (ROM BYTE*)"\nbits:");

memset(temp,0, READER_TEMP);
uitoa(m_skd_data.bit_counter, temp);
TCPPutString(sktHTTP, temp);

TCPPutROMString(sktHTTP, (ROM BYTE*)"\nTM:");
TCPPutString(sktHTTP, GetAsTouchMem());

TCPPutROMString(sktHTTP, (ROM BYTE*)"\nWG:");
TCPPutString(sktHTTP, GetAsWiegand(-1));
}

void HTTPPrint_time(WORD num)
{
	BYTE temp[8];
	int i = 0;
	for(i = 3; i >= 0;i--)
	{
		uitoa(vTick1Reading[i], temp);
		if(strlen(temp)==1)
		{
			temp[1]=temp[0];
			temp[0]='0';
...
{
	TCPPutString(sktHTTP, GetAsWiegandFacilityCode());
}
void HTTPPrint_card_code(void)
{
	TCPPutString(sktHTTP, GetAsWiegandCardCode());
}
struct 
{
	int i;
...
{
  //  <tr>
  //      <td class="style1" colspan="2"><span id="card_list">...</span></td>
  //  </tr>
	DWORD card;
	BYTE temp[8];
	WORD len;
	...
	//if(cur_card.part%2)
	{
		TCPPutROMString(sktHTTP, (ROM BYTE*)"<tr>");
		TCPPutROMString(sktHTTP, (ROM BYTE*)"<td>");
		itoa(cur_card.i, temp);
		TCPPutString(sktHTTP,temp);
		TCPPutROMString(sktHTTP, (ROM BYTE*)"</td>");
...
//	else
	{
		memcpypgm2ram (&card, (const rom void *)&RomCardBuff[cur_card.i*sizeof(DWORD)],sizeof(card));
		TCPPutString(sktHTTP, GetAsWiegand(card));
		TCPPutROMString(sktHTTP, (ROM BYTE*)"</td>");
		TCPPutROMString(sktHTTP, (ROM BYTE*)"<tr>");
		cur_card.i++;
		if(cur_card.i >= 256)
