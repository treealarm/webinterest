...
void ProcessSensor(BYTE bOpen)
{
	if(m_skd_data.sensor != bOpen)
	{
		...
		else
		{
			...
			if(m_autonom)
			{
				//Если автономный режим, то цикл пройден, надо инициализировать состояние
...
void ProcessTimeOuts(void)
{
	...
	if(m_skd_data.rte_open_counter > 0)
	{
		...
		{
			ProcessRTE(0);//Время удержания прошло, убераем состояние "удержание кнопки"
		}
	}
...
void ProcessIO(void)
{
	...
	if(!RTE1)
	{//Кнопка замкнута на землю (нажата)
		ProcessRTE(1);
	}
	ProcessTimeOuts();
